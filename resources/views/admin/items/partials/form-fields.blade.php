<div class="row g-3">
    ...
    @if (in_array('is_active', $columns))
        <div class="col-md-6">
            ...
            <select name="is_active" class="form-select @error('is_active') is-invalid @enderror">
                ...
                <option value="0" {{ old('is_active', $item->is_active ?? 1) == 0 ? 'selected' : '' }}>Nonaktif</option>
            </select>
            @error('is_active')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
    ...
    @if (in_array('photo', $columns))
        <div class="col-md-6">
            ...
            @if (!empty($item?->photo))
                <div class="mt-3">
                    <img src="{{ asset('storage/' . $item->photo) }}" class="photo-preview" alt="Foto Barang">
                </div>
            @endif
        </div>
    @endif
    ...
    @if ($stockColumn && $warehouses->count())
        <div class="col-12">
            <div class="stock-box">
                    ...
                    @foreach ($warehouses as $warehouse)
                        <div class="col-md-4">
                            ...
                            <input type="number"
                                   ...
                                   value="{{ old('stocks.' . $warehouse->id, $stocks[$warehouse->id] ?? 0) }}"
                                   min="0"
                                   class="form-control"
                                   placeholder="0">
                        </div>
